...
public class StackedEngine extends FXMLEngine{

	
	private StackPane stackPane;
	
	
	public StackedEngine() {
        super();
        stackPane = new StackPane();
        setContainer(stackPane);
    }
    ...
    public boolean setScreen(final String name) {
        if (screens.get(name) != null) {   //screen loaded
            ...
            if (!stackPane.getChildren().isEmpty()) {    //if there is more than one screen
            	...
            	stackPane.getChildren().add(screens.get(name));       //no one else been displayed, then just show
                Timeline fadeIn = new Timeline(
                        new KeyFrame(Duration.ZERO, new KeyValue(opacity, 0.0)),
